...
RUN git clone https://github.com/cdcseacave/openMVS.git ${OPENMVS_SRC} && \
    cd ${OPENMVS_SRC} && \
    git checkout ${OPENMVS_VERSION}

# Clone VCG library explicitly (as submodule init is proving unreliable in Docker)
RUN git clone https://github.com/cdcseacave/VCG.git ${OPENMVS_SRC}/vcg
...
# We export VCG_ROOT as requested by OpenMVS CMake FindVCG: "Please specify VCG directory using VCG_ROOT env. variable"
RUN ls -la ${OPENMVS_SRC}/vcg && \
    mkdir -p ${OPENMVS_BUILD} && \
    cd ${OPENMVS_BUILD} && \
    export VCG_ROOT=${OPENMVS_SRC}/vcg && \
    ln -sf /usr/local/cuda/lib64/stubs/libcuda.so /usr/local/cuda/lib64/stubs/libcuda.so.1 && \
    export LD_LIBRARY_PATH=/usr/local/cuda/lib64/stubs:$LD_LIBRARY_PATH && \
    cmake ${OPENMVS_SRC} \
        -DCMAKE_BUILD_TYPE=Release \
        -DCMAKE_INSTALL_PREFIX=/usr/local \
        -DVCG_ROOT=${OPENMVS_SRC}/vcg \
        -DCMAKE_CXX_FLAGS="-O3" \
        -DCUDA_CUDA_LIBRARY=/usr/local/cuda/lib64/stubs/libcuda.so \
        -DOpenMVS_USE_CUDA=ON && \
    make -j$(($(nproc) * 3 / 4)) && \
    make install && \
    # FORCE INSTALL: Manually copy binaries/libs from build dir to /usr/local/
    # (Fixes issue where make install fails to place them in standard PATH)
    cp /opt/openmvs-build/bin/* /usr/local/bin/ && \
    if [ -d "/opt/openmvs-build/lib" ]; then cp -r /opt/openmvs-build/lib/* /usr/local/lib/; fi && \
    ldconfig

# Verify OpenMVS installation
RUN echo "--- Verifying OpenMVS Build ---" && \
    ls -la /usr/local/bin/Densify* || echo "DensifyPointCloud not found in /usr/local/bin" && \
    find /opt/openmvs-build -name DensifyPointCloud || echo "DensifyPointCloud not found in build dir" && \
    DensifyPointCloud --help | head -5 && \
    ReconstructMesh --help | head -5 && \
    ls -la /usr/local/bin/InterfaceCOLMAP && \
    echo "✅ OpenMVS tools installed (Checked)"
...
# Verify binaries in production image
RUN colmap --help | head -5 && \
    DensifyPointCloud --help | head -5 && \
    echo "✅ Production binaries verified"
