...
# We use --load to keep the image in local docker for pushing
docker build --platform=linux/amd64 --no-cache -t "$FULL_IMAGE" -f Dockerfile .
...
echo "3. Important: If the image name hasn_t changed,"
