...
TUNNEL_URL="https://eligibility-hart-baking-conservation.trycloudflare.com"
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"target\": \"production\",
    \"forceNew\": true
  }" | jq -r '"Deployment: " + (.url // "pending")'

echo ""
echo "✅ Vercel updated with tunnel URL!"
echo "Backend: $TUNNEL_URL"
echo "Frontend will redeploy in ~2 minutes"
