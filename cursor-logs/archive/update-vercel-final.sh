...
TUNNEL_URL="https://endless-fiber-amongst-hip.trycloudflare.com"

echo "🔧 Updating Vercel..."

# Get and delete old env var
...
curl -s -X POST \
  ...
  -d "{\"key\":\"NEXT_PUBLIC_API_URL\",\"value\":\"$TUNNEL_URL\",\"target\":[\"production\",\"preview\"],\"type\":\"plain\"}" | jq -r '.value // "updated"'
...
curl -s -X POST \
  ...
  -H "Authorization: Bearer $VERCEL_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"name\":\"$VERCEL_PROJECT_ID\",\"project\":\"$VERCEL_PROJECT_ID\",\"target\":\"production\",\"forceNew\":true}" > /dev/null

echo "✅ Vercel updated: $TUNNEL_URL"
